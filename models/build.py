...
from models import iTransformer, PatchTST, DLinear, OLS, FreTS, MICN, chronos, moirai, timexl, Timer # arima

def build_model(cfg):
    model_name = cfg.MODEL.NAME
    if '-' in model_name:
        model_name = model_name.split('-', 1)[0]
    assert model_name in globals(), f"model {model_name} is not defined"

    model_class = getattr(globals()[model_name], "Model")
